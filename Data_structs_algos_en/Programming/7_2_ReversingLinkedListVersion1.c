...
List Reverse(List L, int K); 	/* Reverse the links of every K elements on L */
...
/* The length of sublist is greater than K, so find the actual head of next 
 * sublist and list it with the last element of last K elements */
int FindPos(List L, int Current, int K)
{
	int i, index;
...
void PrintList(List L)
{
	...
	for (t=L->FirAddr; t != -1; t=L->Arr[t].Next)
		if (L->Arr[t].Next == -1)
